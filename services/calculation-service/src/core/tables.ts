...
export class TableVersionManager {
  ...
  async loadTables(
    ...
    const basePath = `../data/tables/${edition}`;
    
    try {
      const [table2, table4, table5A, table5C] = await Promise.all([
        import(`${basePath}/table2.json`),
        import(`${basePath}/table4.json`),
        import(`${basePath}/table5A.json`),
        import(`${basePath}/table5C.json`),
      ]);
      ...
      throw new Error(`Failed to load ${code} ${edition} tables: ${error}`);
    }
  }
