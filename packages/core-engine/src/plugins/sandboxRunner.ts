...
async function runWithVm2(
  ...
  const vm = new NodeVM({
    ...
    require: opts.allowRequire
      ? {
          ...
          context: 'sandbox',
          root: path.dirname(pluginModulePath), // Restrict to plugin directory
        }
      : {
          external: false,
          builtin: [],
    ...
    ...(opts.memoryLimitMb ? { memoryLimit: opts.memoryLimitMb * 1024 * 1024 } : {}), // Convert MB to bytes
    // Additional security: restrict file system access
    nesting: false, // Disable nested VM creation
  ...
  try {
    ...
    // Try to find the plugin's calculate function
    // Support multiple export patterns:
    // 1. Named export: export { cecSingleDwellingPlugin }
    // 2. Default export: export default cecSingleDwellingPlugin
    // 3. Direct function: export function calculate(...)
    let plugin: any = exported;
    if (exported.default) {
      plugin = exported.default;
    } else if (exported.cecSingleDwellingPlugin) {
      plugin = exported.cecSingleDwellingPlugin;
    }
    
    const computeFn: PluginComputeFn =
      (computeExportName && plugin[computeExportName]) || 
      plugin.calculate || 
      plugin.compute || 
