...
export function computeSingleDwelling(
  ...
  pushStep({ 
    ...
    note: `Method A: Basic load for ${livingArea}m²` 
  });

  const hvacLoadW = calculateHeatingCoolingLoad(
    inputs.heatingLoadW || 0, 
    inputs.coolingLoadW || 0, 
    inputs.isHeatingAcInterlocked || false
  );
  pushStep({ 
    operationId: 'calc_hvac_load', 
    formulaRef: 'CEC 8-200 1)a)iii) + 62-118 3)', 
    intermediateValues: { 
      heating: toFixedDigits(inputs.heatingLoadW || 0), 
      cooling: toFixedDigits(inputs.coolingLoadW || 0) 
    }, 
    output: { loadW: toFixedDigits(hvacLoadW) }, 
    note: 'HVAC Load with demand factors' 
  });
  ...
  if (hasRange) {
    ...
    pushStep({ 
      ...
      intermediateValues: { rating_kW: toFixedDigits(rangeRatingkW) }, 
      output: { loadW: toFixedDigits(rangeDemand) }, 
      note: 'Electric Range Load' 
    });
  }
  ...
  if (otherLargeLoadsW > 0) {
    ...
    pushStep({ 
      ...
      formulaRef: 'CEC 8-200 1)a)viii)', 
      intermediateValues: { 
        totalLargeLoadW: toFixedDigits(otherLargeLoadsW), 
        hasRange: hasRange.toString() 
      }, 
      output: { loadW: toFixedDigits(otherLargeLoadsDemand) }, 
      note: 'Other Large Loads (>1500W)' 
    });
  }
  ...
  if (remainingLoads > 0) {
    pushStep({ 
      operationId: 'calc_remaining_loads', 
      formulaRef: 'CEC 8-200', 
      output: { loadW: toFixedDigits(remainingLoads) }, 
      note: 'Sum of remaining small loads' 
    });
  }
  ...
  pushStep({ 
    ...
    output: { totalW: toFixedDigits(calculatedLoadA) }, 
    note: 'Total calculated load for Method A' 
  });
  ...
  pushStep({ 
    ...
    intermediateValues: { livingArea_m2: toFixedDigits(livingArea) }, 
    output: { minimumW: toFixedDigits(minimumLoadB) }, 
    note: `Method B: Minimum load for area ${livingArea >= 80 ? '≥ 80m²' : '< 80m²'}` 
  });
  ...
  pushStep({ 
    ...
    output: { finalLoadW: toFixedDigits(finalLoadW) }, 
    note: finalLoadNote 
  });

  // 4. Service Current Calculation
  const serviceCurrent_A = finalLoadW / voltageDivisor;
  pushStep({ 
    operationId: 'calc_service_current', 
    formulaRef: phase === 3 ? 'I = P / (V*√3)' : 'I = P/V', 
    output: { serviceCurrentA: toFixedDigits(serviceCurrent_A) }, 
    note: 'Service Current' 
  });
  ...
  pushStep({ 
    ...
    formulaRef: 'CEC T2/T4, T5A, T5C', 
    intermediateValues: { requiredAmps: toFixedDigits(serviceCurrent_A) }, 
    output: { 
      size: conductorResult.size, 
      ampacity: toFixedDigits(conductorResult.effectiveAmpacity || conductorResult.baseAmpacity) 
    }, 
    note: 'Conductor Sizing', 
    tableReferences: conductorResult.tableReferences 
  });

  // 6. Panel/Breaker Sizing
  ...
  const results: CecResults = {
    ...
    continuousAdjustedVA: '0.00', // Placeholder for future continuous load calculations
