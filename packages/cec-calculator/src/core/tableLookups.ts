// @tradespro/calculation-engine/src/core/tableLookups.ts
// TradesPro Multi-Version Table Lookup Engine

import { Table, TableEntry, RuleTables, ConductorResult } from './types';
...
export function lookupConductorSize(
  ...
  const warnings: string[] = [];
  const tableReferences: Array<{ tableId: string; version?: string; rowIndex: number; columnUsed?: string }> = [];
  ...
  if (!table || !Array.isArray(table.entries) || table.entries.length === 0) {
    ...
    return {
      size: 'UNKNOWN',
      baseAmpacity: 0,
      tableReferences: [{ tableId: table?.tableId ?? 'missing', rowIndex: -1 }],
      warnings,
    };
  }
  ...
  if (conductors.length === 0) {
    ...
    return {
      size: 'UNKNOWN',
      baseAmpacity: 0,
      tableReferences: [{ tableId: table.tableId, version: table.version, rowIndex: -1 }],
      warnings,
    };
  }
