 ...
 * Formula:
 * - First 90 m2: 5000W
 * - Additional area: 1000W per 90 m2 portion (rounded up)
 * 
...
export function calculateBaseLoadWithAudit(livingArea_m2: number): {
  ...
  return {
    loadW: totalLoad,
    breakdown: `First 90m2: 5000W + ${additionalPortions} additional portions x 1000W = ${totalLoad}W`
  };
}
...
export function validateLivingArea(livingArea_m2: number): {
  ...
  if (livingArea_m2 > 1000) {
    warnings.push('Living area is unusually large (>1000m2)');
  }
...
export function calculateLivingAreaFromFloors(floors: FloorSpec[]): {
  ...
  for (const floor of floors) {
    let factor = 1;
    let description = `Floor: ${floor.area_m2}m2`;
